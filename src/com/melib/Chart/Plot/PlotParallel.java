 ...
 */
public class PlotParallel extends CPlotBaseMe {
	public PlotParallel() {
	...
	public ActionType GetAction() {
		...
		if (param != null) {
			...
			if ((mp.y - param[0] * mp.x - param[2] >= this.m_lineWidth * -5)
					&& (mp.y - param[0] * mp.x - param[2] <= this.m_lineWidth * 5)) {
				action = ActionType.MOVE;
				return action;
			}
		} else {
	...
	protected float[] GetParallelParams(HashMap<Integer, CPlotMarkMe> pList) {
		if (pList.isEmpty()) {
			return null;
		}
