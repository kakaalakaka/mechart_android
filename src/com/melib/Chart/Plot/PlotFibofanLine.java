 ...
 */
public class PlotFibofanLine extends CPlotBaseMe {
	public PlotFibofanLine() {
	...
	public ActionType GetAction() {
		...
		POINTF firstP = new POINTF(x2, y2 - (y2 - y1) * 0.382F);
		POINTF secondP = new POINTF(x2, y2 - (y2 - y1) * 0.5F);
		POINTF thirdP = new POINTF(x2, y2 - (y2 - y1) * 0.618F);
	...
	protected void Paint(com.melib.Base.CPaintMe paint, HashMap<Integer, CPlotMarkMe> pList, long lineColor) {
		...
		if ((x1 != x2) && (y1 != y2)) {
			...
			listP.addAll(java.util.Arrays.asList(new POINTF[] { firstP, secondP, thirdP }));
